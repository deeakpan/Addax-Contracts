...
// Addax fork: primary prices from DIA Oracle V2 `getValue` instead of Chainlink aggregators.

pragma solidity 0.6.12;

import "../../gmx-contracts/contracts/libraries/math/SafeMath.sol";
import "../../gmx-contracts/contracts/core/interfaces/IVaultPriceFeed.sol";
import "../../gmx-contracts/contracts/oracle/interfaces/ISecondaryPriceFeed.sol";
import "../../gmx-contracts/contracts/amm/interfaces/IPancakePair.sol";
import "../oracle/DIAOracleLib.sol";

contract VaultPriceFeedDia is IVaultPriceFeed {
    ...
    address public gov;
    /// @notice Shared DIA oracle contract (DIAOracleV2) on this chain.
    address public diaOracle;
    /// @notice Reject prices older than this many seconds (LitVM DIA heartbeat = 1h).
    ...
    /// @notice DIA pair key per token, e.g. "LTC/USD".
    mapping (address => string) public diaKeys;
    ...
    function setDiaOracle(address _diaOracle) external onlyGov {
        diaOracle = _diaOracle;
    }

    function setMaxPriceAge(uint256 _maxPriceAge) external onlyGov {
        require(_maxPriceAge > 0, "VaultPriceFeedDia: invalid max age");
        maxPriceAge = _maxPriceAge;
    }

    function setDiaKey(address _token, string calldata _key) external onlyGov {
        diaKeys[_token] = _key;
    }

    function setTokenConfigDia(
        address _token,
    ...
    /// @dev IVaultPriceFeed compatibility: `_priceFeed` may be a DIAPriceFeed adapter; DIA key must still be set.
    ...
    function getLatestPrimaryPrice(address _token) public override view returns (uint256) {
        return getPrimaryPrice(_token, true);
    }

    function getPrimaryPrice(address _token, bool /* _maximise */) public override view returns (uint256) {
        require(diaOracle != address(0), "VaultPriceFeedDia: dia oracle not set");

        string memory key = diaKeys[_token];
        require(bytes(key).length > 0, "VaultPriceFeedDia: dia key not set");

        uint256 decimals = priceDecimals[_token];
        require(decimals > 0, "VaultPriceFeedDia: price decimals not set");

        (uint128 rawPrice, bool inTime) = DIAOracleLib.getPriceIfNotOlderThan(
            diaOracle,
            key,
            uint128(maxPriceAge)
        );
        require(inTime, "VaultPriceFeedDia: stale dia price");
        require(rawPrice > 0, "VaultPriceFeedDia: invalid dia price");

        return uint256(rawPrice).mul(PRICE_PRECISION).div(10 ** decimals);
    }
